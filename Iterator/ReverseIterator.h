//
// Created by djc on 18-9-11.
//

#ifndef MINI_STL_REVERSEITERATOR_H
#define MINI_STL_REVERSEITERATOR_H
...
namespace MINI_STL
{
    ...
    public:
        ...
        reference operator[] (diff_type n){
            return base()[-n - 1];
        }
        reverse_iterator_t operator + (diff_type n)const;
        reverse_iterator_t& operator += (diff_type n);
        reverse_iterator_t operator - (diff_type n)const;
        reverse_iterator_t& operator -= (diff_type n);
    private:
    ...
    public:
        ...
        friend reverse_iterator_t<Iterator> operator + (
                typename reverse_iterator_t<Iterator>::diff_type n,
                const reverse_iterator_t<Iterator>& rev_it);
        template <class Iterator>
        friend typename reverse_iterator_t<Iterator>::diff_type operator- (
                const reverse_iterator_t<Iterator>& lhs,
                const reverse_iterator_t<Iterator>& rhs);
    };// end of reverse_iterator_t

    template<class Iterator>
    reverse_iterator_t<Iterator>& reverse_iterator_t<Iterator>::operator += (diff_type n){
...
template<class Iterator>
reverse_iterator_t<Iterator> reverse_iterator_t<Iterator>::operator + (diff_type n)const{
...
template <class Iterator>
typename reverse_iterator_t<Iterator>::diff_type operator - (
        const reverse_iterator_t<Iterator>& lhs,
        const reverse_iterator_t<Iterator>& rhs){
    return lhs.cur_ - rhs.cur_;
}
...
#endif //MINI_STL_REVERSEITERATOR_H
